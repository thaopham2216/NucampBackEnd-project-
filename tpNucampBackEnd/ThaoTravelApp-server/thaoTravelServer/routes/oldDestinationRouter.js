...
destinationRouter.route('/')
    .get((req, res, next) => {
        Destination.find()
            ...
            .catch(err => next(err)); //this to catch the error and pass it to error handling
    })
    .post(authenticate.verifyUser, authenticate.verifyAdmin, (req, res, next) => {
        Destination.create(req.body)//mongoose will check if it fit the scheme body and send back error
            .then(destination => {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
//find method look for all, findById look for 1 matched id 
        ...
        Destination.findById(req.params.destinationId)
            .then(destination => {
                if (destination) {
                    ...
                    destination.comments.push(req.body); //this only in application memory
                    destination.save() //save this change in mongodb database, save method return a promise so chain .then
                        .then(destination => {
                            res.statusCode = 200;
                            res.setHeader('Content-Type', 'application/json');
                            res.json(destination);
            ...
            .then(destination => {
                if (destination && destination.comments.id(req.params.commentId)) {
                    if ((req.user._id).equals(destination.comments.id(req.params.commentId).author)) {
                        ...
                        destination.save()
                            //.then method use to send back response to the client 
                            .then(destination => {
                                res.statusCode = 200;
                                res.setHeader('Content0-Type', 'application/json');
                                res.json(destination);
                            })
                            .catch(err => next(err));
                    } else {
